...
try {
    ...
    const recentAttendance = user.attendance.filter(att => new Date(att.date) >= oneMonthAgo);

    const checkInCount = recentAttendance.filter(att => att.status === 'Present').length;
